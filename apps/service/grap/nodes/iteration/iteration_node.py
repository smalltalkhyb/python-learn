...
from apps.service.grap.event.event import RunCompletedEvent
from apps.service.grap.event.types import NodeEvent
from apps.service.grap.graph import Graph
from apps.service.grap.models.event import InNodeEvent, IterationRunNextEvent, IterationRunFailedEvent, \
    IterationRunSucceededEvent, BaseNodeEvent, NodeRunStartedEvent, BaseParallelBranchEvent, NodeRunStreamChunkEvent, \
    NodeRunSucceededEvent, BaseGraphEvent, GraphRunFailedEvent, NodeRunFailedEvent, NodeInIterationFailedEvent, \
...
from apps.service.grap.models.variables.variables import NoneVariable, ArrayVariable, IntegerVariable
from apps.service.grap.node_type import NodeType
from apps.service.grap.nodes.iteration import IterationNodeData
from apps.service.grap.nodes.iteration.entities import ErrorHandleMode
from apps.service.grap.nodes.iteration.exc import IteratorVariableNotFoundError, InvalidIteratorValueError, \
    StartNodeIdNotFoundError, IterationGraphNotFoundError, IterationNodeError, IterationIndexNotFoundError
...
class IterationNode(BaseNode[IterationNodeData]):
    ...
    def _run(self) -> Generator[NodeEvent | InNodeEvent, None, None]:
        ...
        #     max_execution_steps=settings.settings.WORKFLOW_MAX_EXECUTION_STEPS,
        #     max_execution_time=settings.settings.WORKFLOW_MAX_EXECUTION_TIME,
        ...
        try:
            if self.node_data.is_parallel:
                ...
                #     max_workers=self.node_data.parallel_nums, max_submit_count=settings.settings.MAX_SUBMIT_COUNT
                # )
                thread_pool=None
    ...
    def _run_single_iter(
        ...
        """
        iter_start_at = datetime.now().replace(tzinfo=None)
                ...
                if isinstance(event, NodeRunSucceededEvent):
                    ...
                    if isinstance(event, NodeRunFailedEvent):
                        if self.node_data.error_handle_mode == ErrorHandleMode.CONTINUE_ON_ERROR:
                            ...
                            duration = (datetime.now().replace(tzinfo=None) - iter_start_at).total_seconds()
